...
class OcrService:
    ...
    def _recognize_with_paddleocr(self, image_bytes: bytes, errors: list[str]) -> dict[str, Any]:
        ...
        try:
            ...
            for variant_name, variant_bytes in self._image_variants(image_bytes, errors, include_rotations=True):
    ...
    def _image_variants(image_bytes: bytes, errors: list[str], include_rotations: bool = False) -> list[tuple[str, bytes]]:
        ...
        try:
            ...
            with Image.open(io.BytesIO(image_bytes)) as img:
                ...
                scale = max(1.0, min(3.5, 2200 / max(rgb.width, 1)))
                if scale > 1.05:
                ...
                if importlib.util.find_spec("cv2") and importlib.util.find_spec("numpy"):
                    ...
                    if coords_crop.size:
                        ...
                        if crop.size and crop.shape[0] > 80 and crop.shape[1] > 80:
                            ...
                            h, w = crop.shape[:2]
                            if h > 240:
                                crop_sources.append(("upper_content_crop", crop[: int(h * 0.72), :]))
                    ...
                    for crop_name, crop_arr in crop_sources:
                        ...
                        target_w = 2200 if w < 1700 else min(w, 2800)
                        factor = max(1.0, min(5.0, target_w / max(w, 1)))
                        resized = cv2.resize(crop_arr, (int(w * factor), int(h * factor)), interpolation=cv2.INTER_CUBIC)
                    ...
                    binary = cv2.adaptiveThreshold(clahe, 255, cv2.ADAPTIVE_THRESH_GAUSSIAN_C, cv2.THRESH_BINARY, 35, 11)
                    ok, encoded = cv2.imencode(".png", binary)
                    if ok:
                        variants.append(("adaptive_threshold", encoded.tobytes()))
                    ...
                    if coords.size:
                        ...
                        if abs(angle) <= 12:
                            ...
                            deskewed = cv2.warpAffine(binary, matrix, (w, h), flags=cv2.INTER_CUBIC, borderMode=cv2.BORDER_REPLICATE)
                            ok, encoded = cv2.imencode(".png", deskewed)
                            if ok:
                                variants.append(("deskewed", encoded.tobytes()))
        except Exception as exc:  # noqa: BLE001
            errors.append(f"图片预处理失败：{exc}")
        return variants
