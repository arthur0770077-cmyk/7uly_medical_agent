...
OCR_BUILD_ID = "ocr-baidu-cloud-20260709"
...
def configure_cpu_runtime() -> None:
    """Keep local OCR fallback inside small-instance CPU limits."""
    for key in ("OMP_NUM_THREADS", "OPENBLAS_NUM_THREADS", "MKL_NUM_THREADS", "NUMEXPR_NUM_THREADS"):
        os.environ.setdefault(key, "1")
    os.environ.setdefault("OCR_MAX_VARIANTS", "1")
...
class OcrService:
    ...
    def __init__(self) -> None:
        ...
        self._baidu_token: str = ""
        self._baidu_token_expire_at = 0.0

    def status(self) -> dict[str, Any]:
        engines = []
        if self._baidu_configured():
            engines.append("baidu_ocr_api")
        ...
        return {
            "build_id": OCR_BUILD_ID,
            "available": engines,
            "preferred": engines[0] if engines else "manual_review",
            "fallback": "manual_review",
            "baidu_ocr_configured": self._baidu_configured(),
            "ocr_max_variants": os.getenv("OCR_MAX_VARIANTS", "1"),
            "quality_plan": [
                "优先解析 PDF 真实文本层，避免截图 OCR 误差。",
                "配置百度 OCR 后，图片优先调用百度云 OCR，避免 Render 免费实例加载本地模型超时。",
                "百度 OCR 不可用时才降级到本地 RapidOCR/PaddleOCR/Tesseract。",
    ...
    def _recognize_image(self, image_bytes: bytes, errors: list[str]) -> dict[str, Any]:
        baidu_result = self._recognize_with_baidu_ocr(image_bytes, errors)
        baidu_text = normalize_ocr_text(baidu_result.get("text", ""))
        if baidu_text:
            baidu_result["text"] = baidu_text
            if rows_from_report_text(baidu_text):
                return baidu_result
        ...
        if baidu_text and best["score"] < 0:
            return baidu_result
        return best

    def _baidu_configured(self) -> bool:
        return bool(self._baidu_credentials()[0] and self._baidu_credentials()[1])

    @staticmethod
    def _baidu_credentials() -> tuple[str, str]:
        api_key = os.getenv("BAIDU_OCR_API_KEY") or os.getenv("BAIDU_API_KEY") or ""
        secret_key = os.getenv("BAIDU_OCR_SECRET_KEY") or os.getenv("BAIDU_SECRET_KEY") or ""
        return api_key.strip(), secret_key.strip()

    def _baidu_access_token(self, errors: list[str]) -> str:
        if self._baidu_token and time.time() < self._baidu_token_expire_at - 120:
            return self._baidu_token
        api_key, secret_key = self._baidu_credentials()
        if not api_key or not secret_key:
            return ""
        token_url = "https://aip.baidubce.com/oauth/2.0/token?" + urlencode(
            {
        ...
        try:
            req = Request(token_url, method="POST")
            with urlopen(req, timeout=12) as resp:
                data = json.loads(resp.read().decode("utf-8", "replace"))
            token = str(data.get("access_token") or "")
            if not token:
                errors.append(f"Baidu OCR token error: {data.get('error_description') or data.get('error') or 'empty token'}")
                return ""
            self._baidu_token = token
            self._baidu_token_expire_at = time.time() + float(data.get("expires_in") or 2592000)
            return token
        except Exception as exc:  # noqa: BLE001
            errors.append(f"Baidu OCR token request failed: {exc}")
            return ""

    def _recognize_with_baidu_ocr(self, image_bytes: bytes, errors: list[str]) -> dict[str, Any]:
        if not self._baidu_configured():
            return {"text": "", "confidence": None, "engine": "baidu_ocr_api"}
        token = self._baidu_access_token(errors)
        if not token:
            return {"text": "", "confidence": None, "engine": "baidu_ocr_api"}
        endpoint = os.getenv("BAIDU_OCR_ENDPOINT", "https://aip.baidubce.com/rest/2.0/ocr/v1/general_basic")
        params = {
        ...
        try:
            req = Request(
                endpoint + "?" + urlencode({"access_token": token}),
            ...
            if data.get("error_code"):
                errors.append(f"Baidu OCR error {data.get('error_code')}: {data.get('error_msg')}")
                return {"text": "", "confidence": None, "engine": "baidu_ocr_api"}
            lines = [str(item.get("words") or "").strip() for item in data.get("words_result", [])]
            text = normalize_ocr_text("\n".join(line for line in lines if line))
            return {"text": text, "confidence": None, "engine": "baidu_ocr_api"}
        except Exception as exc:  # noqa: BLE001
            errors.append(f"Baidu OCR request failed: {exc}")
            return {"text": "", "confidence": None, "engine": "baidu_ocr_api"}
    ...
    def _image_variants(image_bytes: bytes, errors: list[str], include_rotations: bool = False) -> list[tuple[str, bytes]]:
        ...
        try:
            ...
            with Image.open(io.BytesIO(image_bytes)) as img:
                ...
                max_variants = max(1, min(6, int(os.getenv("OCR_MAX_VARIANTS", "1"))))
